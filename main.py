...
loss_cycle_a = tf.reduce_mean(
    tf.reduce_mean(tf.abs(a - a_identity),
                   reduction_indices=[1, 2, 3]))  # following the paper implementation.(divide by #pixels)
loss_cycle_b = tf.reduce_mean(
    tf.reduce_mean(tf.abs(b - b_identity),
                   reduction_indices=[1, 2, 3]))  # following the paper implementation.(divide by #pixels)
...
try:
    ...
    for step in range(MAX_ITERATION + 1):
        ...
        print(f'{step:7d} : W : {W_eval:1.6f}, '
              f'GP : {GP_eval:1.6f}, '
              f'Loss G : {loss_g_eval:1.6f}, '
              f'Loss Cycle : {loss_cycle_eval:1.6f}')
